...
def _google_drive_configured() -> bool:
    return bool(
        ...
        and (os.getenv("GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON") or os.getenv("GOOGLE_DRIVE_SERVICE_ACCOUNT_FILE"))
    )

def _google_drive_credentials_info():
...
def _google_drive_upload(stream, filename: str, content_type: str):
    ...
    credentials = service_account.Credentials.from_service_account_info(
        _google_drive_credentials_info(),
    ...
    if os.getenv("GOOGLE_DRIVE_PUBLIC", "true").lower() in {"1", "true", "yes"}:
...
async def upload_file(
    ...
    try:
        ...
        if wants_google_drive:
            raise HTTPException(
                status_code=503,
                detail="Google Drive upload is not configured. Add GOOGLE_DRIVE_FOLDER_ID and GOOGLE_DRIVE_SERVICE_ACCOUNT_JSON.",
            )
